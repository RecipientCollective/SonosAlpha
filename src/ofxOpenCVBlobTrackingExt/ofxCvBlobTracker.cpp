...
// note here that we take a pointer to an ofxVec3f, not a reference
// this way we can be sure of changing the particular instance that
// has been passed into the method.
void ofxCvBlobTracker::findVelocity( int id, ofVec3f* vec) {
	ofxCvTrackedBlob blob = getById(id);
	ofxCvTrackedBlob oldBlob;
	int i;
	...
	vec->x = (blob.centroid.x - oldBlob.centroid.x);
	vec->y = (blob.centroid.y - oldBlob.centroid.y);
